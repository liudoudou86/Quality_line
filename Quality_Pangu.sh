...
function check_mysql_bak(){
	...
	if [ -d $check_bak_dir -a  -d $check_bak_file -a -d $check_bak_sh ];then
		...
		if [ $count -gt 0 -a $count -le 3 ] ;then
			file_check=0
		fi
	fi
...
function nginx_sercurity_daemon(){
  echo -e "测试开始："
  check_time=0
  nginx_sercurity_pid=`ps -ef | grep "/opt/td-security/nginx-waf/sbin/nginx_sercurity" | grep -v grep | awk '{print $2}'`
  sleep 1
  echo -e "开始杀死nginx进程！进程号：$nginx_sercurity_pid"
  kill -9 $nginx_sercurity_pid
  sleep 1
  nginx_sercurity_pid=`ps -ef | grep "/opt/td-security/nginx-waf/sbin/nginx_sercurity" | grep -v grep | awk '{print $2}'`
